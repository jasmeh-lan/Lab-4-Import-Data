...
```{r}
#| label: load-data
buoy <- read_table("Data/Data-Raw/buoy_2024.txt")
buoy
```
...
write_csv(buoy_clean, "Data/Data-Raw/buoy_update.csv")
```
...
buoy_clean <- read_csv("Data/Data-Raw/buoy_update.csv", 
...
#| label: parse-double
#| eval: false
buoy_final <- buoy_clean |> 
  mutate(across(.cols = everything(), ~parse_double(.x)))

#write_csv(buoy_final, "Data/Data-Clean/buoy_final.csv")
```
...
buoy_final <- read.csv("Data/Data-Clean/buoy_final.csv")

buoy_final |>
  ggplot(mapping = aes(x = mm,
                       y = atmp,
                       alpha = 0.5,
                       color = mm)) +
  ...
  labs(x = "Month in 2024", 
       y = "Air Temperature (Celisus)", 
       title = "Monterey Bay Caynon Air Temperature in 2024") +
  theme(legend.position = "none")

#ggsave("Monterey-AirTemp-Month.png")
```
...
> Data about Water Insecurity and the various factors that cause water insecurity
> What states in the United States are water insecured?
> How does the relation of water insecurity have to do with ethnicity and social status?
